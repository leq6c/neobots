...
def calculate_token_sales(
    ...
    sell_probability_modifier = max(
        0,
        (price_ratio - parameters.PRICE_THRESHOLD_FOR_INCREASED_SELLING)
        * parameters.SELL_PROBABILITY_INCREASE_WITH_PRICE,
    )
    ...
    sell_probability = (
        parameters.BASE_SELL_PROBABILITY + sell_probability_modifier * sentiment_factor
    )
...
def calculate_token_purchases(
    ...
    if len(token_value_history) >= parameters.CONSECUTIVE_INCREASES_FOR_TREND:
        ...
        for i in range(1, parameters.CONSECUTIVE_INCREASES_FOR_TREND + 1):
            if i >= len(token_value_history):
                break
            if token_value_history[-i] > token_value_history[-i - 1] * (
                1 + parameters.PRICE_INCREASE_THRESHOLD
            ):
                increases += 1
    ...
    for _ in range(total_buyers):
        ...
        variance = random.uniform(
            -parameters.MAX_PURCHASE_VARIANCE, parameters.MAX_PURCHASE_VARIANCE
        )
...
def simulate_additional_ap_purchases(
    active_bots, token_value, market_sentiment, ap_types, parameters=None
):
    """
    ...
    adjusted_purchase_rate = parameters.ADDITIONAL_AP_PURCHASE_BASE_RATE * (
        1 + (market_sentiment - 0.5) * parameters.MARKET_SENTIMENT_EFFECT_ON_PURCHASE
    )

    # Add variation to purchase rate
    purchase_rate = max(
        0,
        adjusted_purchase_rate
        + random.uniform(
            -parameters.ADDITIONAL_AP_PURCHASE_VARIANCE,
            parameters.ADDITIONAL_AP_PURCHASE_VARIANCE,
        ),
    )
    ...
    if token_value < 0.001:
        total_tokens_spent *= (
            token_value * 1000
        )  # Decrease purchases as value decreases
...
def calculate_external_post_fee(ap_types, active_bots, parameters=None):
    ...
    total_ap_distribution = sum(
        ap_data["DISTRIBUTION"] for ap_data in ap_types.values()
    )
    total_points = sum(
        max(ap_data["EARNING"], ap_data["GIVING"]) for ap_data in ap_types.values()
    )

    # Base fee per user = distribution × total points
    base_fee = (
        total_ap_distribution * total_points * parameters.EXTERNAL_POST_PRICE_FACTOR
    )
...
def simulate_external_post_usage(
    total_bots, token_value, market_sentiment, ap_types, parameters=None
):
    """
    ...
    progress = total_bots / parameters.MAX_BOTS
    adjusted_progress = progress**2
    external_users = int(
        parameters.MAX_BOTS * adjusted_progress * parameters.EXTERNAL_POST_USER_RATIO
    )

    # Adjust base usage rate by market sentiment
    adjusted_usage_rate = parameters.EXTERNAL_POST_USAGE_BASE_RATE * (
        1 + (market_sentiment - 0.5) * 0.3
    )
