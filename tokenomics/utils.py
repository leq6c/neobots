...
def calculate_round_duration(bot_count, parameters=None):
    ...
    return max(0.05, 0.01 * (bot_count**0.67625))
...
def simulate_bot_growth(current_bots, elapsed_hours, cumulative_hours, parameters=None):
    ...
    target_bots_now = parameters.MAX_BOTS / (
        1
        + math.exp(
            -parameters.GROWTH_STEEPNESS
            * (cumulative_hours - parameters.MID_POINT_HOURS)
            / parameters.BOT_GROWTH_TARGET_HOURS
        )
    )

    # Theoretical number of BOTs in the previous round
    target_bots_prev = parameters.MAX_BOTS / (
        1
        + math.exp(
            -parameters.GROWTH_STEEPNESS
            * ((cumulative_hours - elapsed_hours) - parameters.MID_POINT_HOURS)
            / parameters.BOT_GROWTH_TARGET_HOURS
        )
    )
...
def calculate_active_bots(total_bots, cumulative_hours, parameters=None):
    ...
    activity_rate = max(
        parameters.MIN_ACTIVITY,
        1 - parameters.ACTIVITY_DECAY_PER_HOUR * cumulative_hours,
    )
    return max(1, int(total_bots * activity_rate))


def calculate_dynamic_token_rate(
    current_total_bots,
    active_bots_history,
    previous_token_rate,
    round_duration_in_hours,
    parameters=None,
):
    """
    ...
    """
    start_decay = 100_000
    if current_total_bots > start_decay:
        progress = (current_total_bots - start_decay) / (
            (parameters.MAX_BOTS * 0.8) - start_decay
        )
        eased = progress**2
        factor = 1 - 0.5 * eased
        if factor < 0.5:
            factor = 0.5
        return parameters.BASE_TOKEN_RATE * factor
    ...
    if short_term_change > parameters.SIGNIFICANT_CHANGE_THRESHOLD:  # User increase
        new_token_rate = parameters.MAX_TOKEN_RATE - (
            parameters.MAX_TOKEN_RATE - previous_token_rate
        ) * math.exp(-0.03)
    else:
        new_token_rate = parameters.MIN_TOKEN_RATE + (
            previous_token_rate - parameters.MIN_TOKEN_RATE
        ) * math.pow(0.8, 0.17)

    # Limit rate range
    new_token_rate = max(
        parameters.MIN_TOKEN_RATE, min(parameters.MAX_TOKEN_RATE, new_token_rate)
    )

    return new_token_rate


def calculate_points_per_bot_per_hour(
    active_bots, ap_type_data, round_duration, parameters=None
):
    """
    ...
    self_points_per_bot = (
        ap_type_data["DISTRIBUTION"]
        * ap_type_data["USAGE_PROB"]
        * ap_type_data["EARNING"]
    )
...
def calculate_minimum_roi_time(
    nft_price, token_value, points_per_hour_data, parameters=None
):
    """
...
def update_market_sentiment(previous_sentiment, token_value_change, parameters=None):
    ...
    random_factor = random.uniform(
        -parameters.MARKET_SENTIMENT_VOLATILITY, parameters.MARKET_SENTIMENT_VOLATILITY
    )
