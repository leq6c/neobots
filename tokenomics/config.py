...
class SimulationParameters:
    ...
    def __init__(self):
        ...
        self.INITIAL_NFT_SALES = self.BASE_PRICE * self.INITIAL_BOTS  # Initial NFT sales
        ...
        # Simulation time
        self.TARGET_HOURS = 8760        # Target time (3 years = 365 days * 24 hours * 3)

        # Parameters for BOT growth simulation
        self.MAX_BOTS = 100_000 # Maximum number of BOTs
        self.BOT_GROWTH_TARGET_HOURS = 8760  # Target time (3 years = 365 days * 24 hours * 3)
        # Sigmoid growth parameters
        self.MID_POINT_HOURS = 4380     # Inflection point (1.5 years)
        ...
        self.BASE_SELL_PROBABILITY = 0.05  # Base probability of a BOT selling tokens in a round
        self.MAX_SELL_PERCENTAGE = 0.2  # Maximum percentage of tokens a BOT can sell at once
        self.SELL_PROBABILITY_INCREASE_WITH_PRICE = 0.02  # Increase sell probability as price increases
        self.PRICE_THRESHOLD_FOR_INCREASED_SELLING = 2.0  # Price threshold compared to initial price
        self.MARKET_SENTIMENT_VOLATILITY = 0.2  # How much market sentiment changes between rounds
        self.INITIAL_MARKET_SENTIMENT = 0.5  # Initial market sentiment (0.0-1.0)

        # Token buying parameters
        self.BOT_THRESHOLD_FOR_BUY_PRESSURE = 50  # Minimum number of BOTs to start buy pressure
        self.BASE_BUY_PROBABILITY = 0.03  # Base probability of external purchase in a round
        self.UPTREND_BUY_MULTIPLIER = 3.0  # Multiplier for buy probability during uptrends
        self.PRICE_INCREASE_THRESHOLD = 0.05  # Percentage increase to identify uptrend
        self.CONSECUTIVE_INCREASES_FOR_TREND = 3  # Number of consecutive increases to confirm trend
        ...
        self.ADDITIONAL_AP_PURCHASE_BASE_RATE = 0.3  # Base purchase rate for active users
        self.ADDITIONAL_AP_PURCHASE_VARIANCE = 0.1  # Purchase rate variation
        self.ADDITIONAL_AP_PURCHASE_PRICE_FACTOR = 1.5  # AP price calculation factor
        self.MARKET_SENTIMENT_EFFECT_ON_PURCHASE = 0.5  # Effect of market sentiment on purchase rate
        self.TOKEN_BURN_PERCENTAGE = 1.0  # Percentage of token payments to burn (100%)

        # External user POST usage parameters
        self.EXTERNAL_POST_USER_RATIO = 0.8  # Ratio of external POST users to BOT count
        self.EXTERNAL_POST_USAGE_BASE_RATE = 0.6  # Base usage rate for external users
        ...
        self.USER_DECLINE_SENSITIVITY = 1.2   # Sensitivity to user decline (more sensitive)
        self.SMOOTHING_FACTOR = 0.3           # Coefficient to smooth changes (0-1, lower is more gradual)

        # Trend detection parameters
        self.TREND_WINDOW_SIZE = 5            # Number of past rounds used for trend detection
        self.SIGNIFICANT_CHANGE_THRESHOLD = 0.0001  # Threshold for significant change (0.1%)
